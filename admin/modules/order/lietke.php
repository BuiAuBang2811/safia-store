...
if (isset($_GET['order_status'])) {
    ...
    $sql_order_list = "SELECT * FROM orders JOIN account ON orders.account_id = account.account_id WHERE orders.order_type != 5 AND orders.order_status = $order_status ORDER BY orders.order_id DESC LIMIT $begin,10";
    $query_order_list = mysqli_query($mysqli, $sql_order_list);
} else {
    $url_status = '';
    $sql_order_list = "SELECT * FROM orders JOIN account ON orders.account_id = account.account_id WHERE orders.order_type != 5 AND orders.order_status >= 0 AND orders.order_status < 3 ORDER BY orders.order_id DESC LIMIT $begin,10";
    $query_order_list = mysqli_query($mysqli, $sql_order_list);
}
?>
<div class="row">
...
<?php
if (isset($_GET['order_status']) && $_GET['order_status'] == 3) {
?>

<?php
} else if (isset($_GET['order_status']) && $_GET['order_status'] == -1) {
?>
<?php
} else {
?>
    <div class="dialog__control">
        <div class="control__box">
            ...
            <a href="modules/order/xuly.php?cancel=1" class="button__control" onclick="return confirm('Bạn có thực sự muốn hủy đơn hàng này không?')" id="btnCancel">Hủy đơn hàng</a>
        </div>
    </div>
<?php
}
