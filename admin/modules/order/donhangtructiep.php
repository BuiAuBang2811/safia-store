...
if (isset($_GET['order_status'])) {
    ...
    $sql_order_list = "SELECT * FROM orders JOIN account ON orders.account_id = account.account_id WHERE orders.order_status = $order_status AND orders.order_type = 5 ORDER BY orders.order_id DESC LIMIT $begin,10";
    $query_order_list = mysqli_query($mysqli, $sql_order_list);
} else {
    $url_status = '';
    $sql_order_list = "SELECT * FROM orders JOIN account ON orders.account_id = account.account_id WHERE orders.order_status > 0 AND orders.order_type = 5 ORDER BY orders.order_id DESC LIMIT $begin,10";
    $query_order_list = mysqli_query($mysqli, $sql_order_list);
}
?>
<div class="row">
                ...
                <div class="main-pane-top d-flex justify-center align-center">
                    ...
                    <div class="dropdown dropdown__item">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuSizeButton2">
                            <a class="dropdown-item" href="index.php?action=order&query=order_live">Đã hoàn thành</a>
                            <a class="dropdown-item" href="index.php?action=order&query=order_live&order_status=-1">Đã hủy</a>
                        </div>
                    </div>
                </div>
...
<div class="dialog__control">
    <div class="control__box">
        <a href="modules/order/xuly.php?cancellive=1" class="button__control" onclick="return confirm('Bạn có thực sự muốn hủy đơn hàng này không?')" id="btnCancel">Hủy đơn hàng</a>
    </div>
</div>
<script>
    ...
    function getCheckedCheckboxes() {
        ...
        btnCancel.href = "modules/order/xuly.php?cancellive=1&data=" + JSON.stringify(checkedIds);
    }
</script>
